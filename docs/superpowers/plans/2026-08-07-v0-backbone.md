...
def synthetic_gpx(n=10, with_times=False, with_ele=False):
    ...
    for i in range(n):
        ...
        if with_ele:
            p.elevation = 100.0 + i
        if with_times:
            # timedelta, not the seconds argument -- i*30 overflows at 60
            p.time = (datetime.datetime(2021, 3, 12, 9, 0, 0,
...
describe('supabase config resolution', () => {
  ...
  it('falls back to the local stack in dev', async () => {
    ...
    // the JWT is base64url; assert against the DECODED payload, not the raw string
    expect(JSON.parse(atob(cfg.anonKey.split('.')[1])).iss).toBe('supabase-demo');
  });
...
**Amended after execution (commit `bb9ddc8`):** the single-file version below trips
`react-refresh/only-export-components` as a hard error. The committed shape splits it:
`auth.tsx` keeps `AuthProvider` (component export only) and a new `auth-context.ts`
holds `AuthContext`/`useAuth`. Import `useAuth` from `lib/auth-context`.
...
In `web/src/components/Layout.tsx`, add a sign-in/out affordance next to the download button, hoisting `useAuth()` to the top of the `Layout` component (hooks at top level):

```tsx
    const { user, signOut } = useAuth();
```

then in the header controls:
...
with `import { Link } from 'react-router-dom';` and `import { useAuth } from '../lib/auth';` added.
...
5. Confirm you land back on the app signed in, and the session persists across a reload.
...
- [ ] **Step 4: set server env on Railway** — `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_JWT_SECRET` (all from Settings → API), plus `ALLOWED_ORIGINS` set to the real site domains.
...
The annotated-sheet UI (trip pages, landing, sharing controls, upload form) — that is the **face plan** (`2026-08-07-v0-face.md`), which renders exclusively through Task 7's query layer. Chat and all AI features — phase 4, after the face. Photo paths B/C — phase 5.
